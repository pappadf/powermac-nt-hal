...
[E24] — and that is exactly why the ten variables are **not** stored that way: the veneer honours
`OSLOADER` over its own boot path, so with them in NVRAM `BOOT.OF` stopped booting the CD and
showed the disk's boot menu instead [E26]. `BOOTDISK.OF` sets them as `/options` properties with
`(property)`, which lives until the next reset, for the boot it starts; `BOOT.OF` sets none and
the veneer takes `\PPC\SETUPLDR` off the CD as before. The veneer's `read_ARC_env_vars`
overwrites every entry of its argv table from whatever is there. Nothing is injected, no
breakpoint, no poke: the old rig's ten pokes at `OSLOADER`'s first query (ledger row 16) and its
relocation of the loader path (wall 47) both existed only because that rig had no environment at
all. Ledger rows 11, 12 and 16 are retired.
...
| **E21** | **`BOOT.OF` reads the whole disk into RAM, and the order of the mappings matters** | Typed one line at a time: 90 × `ADDR BLK 20 " read-blocks" fd $call-method .` each answer `20`, the sum `B40`, and `3A01000 c@` → `EB` — the OEM image is where `include/oemdisk.h` says. The eval'd script then printed `powermac-nt-hal: B40 blocks of the OEM disk are in RAM` and **`CLAIM failed`**: with the OEM region claimed first, the veneer's `3E00000 27800 map-space` fails. Probed in the other order, all four `map-space` calls succeed; the script was reordered (§4.5) |
...
| **E25** | **The installed system boots from the floppy, no pokes** | `load fd:,\bootdisk.of` + eval: the veneer's menu with *Windows NT Workstation Version 4.00*, `OS Loader V4.00`, the kernel banner, this HAL with `setup block 00000000` and `system path -> 'D:\WINNT'`, `adbport.sys` as a boot driver from Setup's registry (`mouse class connected`, `keyboard class connected`), drive letters C: and D:, and `autochk` checking D: (§2.3, run of 16 September 13:53) |
| **E26** | **A stored ARC environment breaks the CD boot; a per-boot one does not** | With the ten variables in NVRAM (`setenv` in `SETUP.OF`, first draft), `BOOT.OF` — the CD boot — brought up the veneer's *disk* menu and `Booting from '…partition(1)\os\winnt40\osloader.exe'` → `VrOpen returned 8`: the veneer prefers `OSLOADER` to its own boot path. `" x" encode-string " TESTP" " /options" find-package drop (property)` creates a property `printenv` does not list and `reset-all` clears; `BOOTDISK.OF` sets the environment that way, for its boot only. `delete-property` exists too |
| **E27** | **`STOP 0x35` as GUI Setup starts: `mouclass` enabling a port `adbport` never announced** | The IRP at the STOP is 0x70 bytes — no stack locations — so it was built against a device object with `StackSize` 0. A conditional breakpoint on `IoAllocateIrp` with `r3 == 0` caught it: the caller is `IoBuildDeviceIoControlRequest` (`lr` = entry+0x48), internal, no buffers, an event on the stack — an `IOCTL_INTERNAL_MOUSE_ENABLE` — from a frame whose saved `r24` is `\Driver\Mouclass`'s class device. NT 4 class drivers build their port lists from `HARDWARE\DEVICEMAP\KeyboardPort` and `PointerPort`, which every port driver writes (`usbadb.sys` has the strings; `adbport` had none); `mouclass` still found `\Device\PointerPort0` by name to connect, then enabled a port pointer that was not ours. `RtlWriteRegistryValue(RTL_REGISTRY_DEVICEMAP, …)` for both ports in `DriverEntry`, and the boot goes on into GUI Setup (run of 16 September 16:26; `floppy.sys` and the HAL's partition exports were suspected first and cleared) |
| **E28** | **GUI Setup's fills never appear: `cirrus.dll` drives the GD5430's BitBLT engine through memory-mapped registers in the legacy VGA window** | With the STOP gone, GUI Setup starts, loads its palette and draws one grey *< Back*; no fill ever appears and nothing more is drawn. A dump of the whole 1 MB of VRAM at the end of the run holds ~2 KB of non-zero bytes: the HAL console's text, the button, and at VRAM `0x18000` — the window's `0xB8000` — a **BLT register block**: `ff ff ff ff`, pitches, start addresses, a ROP byte (`0x59`), and `0x02`, the start bit, in the `+0x40` start/status register. The driver's first faults, before the window was aliased, were `stb 0x40(r7)` / `lbz 0x40(r11)` at that very offset. The emulated 54M30 has no BLT engine (its own comment says so) and no legacy window, so the writes land as pixels and the status byte never clears: the DLL spins. The palette is right (6-bit DAC, `$00` in the hidden DAC register, Windows' index 1 is dark red — hence the red console text), no GR31 start is ever written (the engine is reached by MMIO, not by I/O), and the model's graphics-register file was 32 entries (GR20–GR35 aliased onto GR0–GR15; fixed in the emulator). On 09-14 the window was unmapped, the miniport reported no MMIO, and the DLL filled on the CPU |
...
* **`4000 1000 map-space` after `init-program` is not a diagnostic.** `mkcoldboot.py` does it and
  ...
  instrumentation.
* **The OEM disk's `map-space` goes last, after the veneer's two.** `map-space` is
  `2dup 0 claim-mem drop 2dup 0 claim-virt drop over swap 0 do-map` (`see map-space` inside the
  package). Claim the 0x169000-byte OEM region first and the later `3E00000 27800 map-space` for
  the veneer answers **`CLAIM failed`** and the definition aborts — before `go`, and with nothing
  else on the console, so from outside it is the eval doing nothing for 87 G instructions [E21].
  Claim it after `3D00000`, `3E00000` and `4000`, and all four go through. Why the firmware's
  `claim-mem` cares about the order is not known; the order is.
* **And the OEM region goes right below the staging area, `0x3B97000`, not just anywhere.** Claimed in
  the middle of free memory it adds an entry to `/memory`'s `available` list, and with that entry
  the veneer answers SETUPLDR's fourth open of the CD with an empty firmware path — Setup asks for
  the CD "in Drive A:" and never gets past it [E22]. Adjacent to the veneer's own claim it adds
  nothing, and Setup proceeds. `include/oemdisk.h` carries the address and the reasoning; the
  `-vrdebug` switch that found it is in §4.8.
...
### 4.7 C5: `adbport.sys`, and the OEM disk that rides in RAM — **works; text-mode Setup completes**

Written on 16 September; loaded the same day, after the three faults of E21, E22 and E23 were
taken out of the way. The run of 16 September 11:49 (`tmp/test8.log`, screenshot
[`2026-09-16-text-mode-setup-complete.png`](../traces/2026-09-16-text-mode-setup-complete.png))
goes from the two typed lines to *"This portion of Setup has completed successfully. Press ENTER
to restart your computer."* — the OEM-file copy of §4.6, which ended every earlier attempt, went
through off the RAM floppy. What it is:
...
**What the first test had to show**, in order — and what it showed, in the serial log:

```
HAL: OEM disk: looking at 03b97000 (dbat1 00000000 dbat2 00000000)
HAL: OEM disk: pages 00003b97..00003d00 were type 5 - now firmware-permanent
HAL: OEM disk: 1474560 bytes at 03b98000, 2880 blocks of 512
...
HAL: module adbport.sys at 806e7000 size 00008000
...
adbport: ADB keyboard, mouse and OEM disk for the Apple Network Server (powermac-nt-hal)
adbport: OEM disk 1474560 bytes at 3b98000 mapped at e7458000
HAL: ADB autopoll on
adbport: up; keyboard, pointer, OEM disk as \Device\Floppy0
adbport: keyboard class connected, service 8073b4bc
...
HAL: IoAssignDriveLetters: boot device '\Device\CdRom0' -> E:, 1 floppy 1 disk 1 cdrom
...
HAL: HalReturnToFirmware(3) — halted
```

Every keystroke from the Welcome screen to the restart prompt came from the ADB keyboard through
this driver; `i8042prt.sys` is still loaded from the CD beside it and does nothing. The three
faults between the draft and this run, each one silent from the outside:

1. **`CLAIM failed`** — the OEM region claimed before the veneer's mappings (E21, §4.5).
2. **`OFOpen('')`** — the OEM region claimed in the middle of free memory (E22, §4.5); found
   with `-vrdebug` (§4.8).
3. **A hang after `VGA aperture … left alone`** — `KePhase0MapIo` hands out addresses inside the
   window the HAL's own DBAT3 maps; two BATs on one address is undefined on the 604. The HAL now
   borrows DBAT2 for the header read (E23). The same trace showed the veneer typing everything
   above 8 MB *FirmwareTemporary*, which the kernel frees after boot — so the retype covers that
   type, not only Free.

**What is not proved yet:** the reboot. Setup's restart prompt returns to the firmware
(`HalReturnToFirmware(3)`); booting the installed disk is the next phase (§6), and that boot
carries neither the floppy's OEM disk nor `adbport.sys`'s registry entry unless Setup wrote one
— the first thing to read off the installed disk.

### 4.8 `-vrdebug`: the veneer's own trace, switched on from `bootargs`

The veneer's `parse_args` walks `/chosen bootargs` with `strctok` and, on `-vrdebug`, feeds
the next token to its `atoi` (decimal, or hex with a `0x` prefix) and stores it in `VrDebug`.
So no poke and no patch is needed — one line in `BOOT.OF` before `go`:

```forth
" -vrdebug 530" encode-string " bootargs" _chosen (property)
```

530 is 0x212. The bits seen so far, from the calls to `debug` in the disassembly:

| bit | prints |
|---|---|
| `0x0002` | every Open Firmware client call (`OFOpen('…')`, `OFOpen: IHandle: …`, …) |
| `0x0008` | the OBP → ARC tree conversion |
| `0x0010` | `VrCreateMemoryDescriptors`: the `reg`/`available` ranges and the descriptor list with types |
| `0x0020` | argument parsing (`OsLoader '…'`, `-vrdebug: '…'`) |
| `0x0040` | `VrMemoryInitialize` |
| `0x0100` | `NodeToPath` |
| `0x0200` | `VrOpen` / `VrClose` entry and exit, with the ARC path and the FileId |

The output goes to the firmware console — `ttya` here — so it lands in the same serial log as
everything else, ahead of SETUPLDR's first screen. `tmp/boot-dbg.of` in the rig is the shipped
`BOOT.OF` with that one line added; `mkbootfloppy.py --boot-script` puts it on a disk. This is
what showed the empty path of E22 in one run, after four runs of guessing.
