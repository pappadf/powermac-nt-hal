...
| [`2026-09-17-the-bitblt-engine.md`](2026-09-17-the-bitblt-engine.md) | **GUI Setup draws.** The GD5430's BitBLT engine and the legacy VGA window, modelled and mirrored into an address this board's CPU can reach. Four places the specification differed from the manual, the byte-lane question answered against both of its candidates -- the XOR was the dump tool, not the driver -- a screen that matches a CPU-drawn reference in 0 of 307,200 pixels, thousands of blits from a real driver, and a complete installation built from scratch -- text-mode Setup, the GUI wizard, and the finished system booting to its logon screen |
