...
| 10 | The driver written over `\PPC\I8042PRT.SYS` — **and on a stock CD there is no replacement for it** | it abuses a name SETUPLDR hardcodes. An earlier claim here that the OEM disk retires it was wrong: SETUPLDR has OEM prompts for SCSI, Computer and Display and **none for the keyboard** (two `SlLoadOemDriver` call sites, neither of them a keyboard), so `txtsetup.oem`'s `[Keyboard]` section is `setupdd.sys`'s, read under NT long after Setup needs a keyboard | a boot medium we control that is also big enough to be Setup's *source* — because the boot medium is the source medium (measured). A 1.44 MB floppy is not; a small FAT partition on the install disk would be |
