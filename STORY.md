...
| 11 | ~~`SYSTEMPARTITION` synthesised by the HAL from the loader's ARC disk list (wall 35)~~ **retired (16 September)** | on real ARC hardware it is NVRAM, written by `ARCINST.EXE`; the HAL guessing it is policy in the wrong place | done: the veneer reads every ARC variable as a property of Open Firmware's `/options` node -- which *is* this machine's NVRAM -- and `mkbootfloppy.py`'s `BOOTDISK.OF` puts the ten of them there with `(property)` for the boot it starts (the boot-floppy note, §2.3). Not NVRAM: `setenv` would persist them, and a persisted `OSLOADER` turns the CD boot into a boot of the disk (E26). The HAL's synthesis stays as a fallback |
| 12 | ~~The ARC environment does not survive a reboot~~ **retired (16 September)** | it was plain memory in the HAL | done, see row 11: the floppy is the store, and every boot sets what that boot needs. NVRAM would hold them (`setenv OSLOADER …` survives `reset-all`, probed) but must not, E26 |
...
| 16 | ~~The ARC environment injected into the veneer at OSLOADER's first query (wall 45 onward)~~ **retired (16 September)** | a real machine reads it from NVRAM; this wrote ten variables into the veneer's own table from outside | done, see row 11: `read_ARC_env_vars` overwrites every argv slot with the `/options` property of the same name, so nothing has to be injected and wall 47's path relocation is not needed either |
