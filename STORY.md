...
| 10 | ~~The driver written over `\PPC\I8042PRT.SYS`~~ **retired (15 September)** | it abused a name SETUPLDR hardcodes | the same OEM disk: `I8042PRT.SYS` is a file in its root, named by `txtsetup.oem`'s `[Files.Keyboard.…]`. The `[Computer]` half of that file is exercised; the `[Keyboard]` half is written but Setup has not yet been made to ask for it |
