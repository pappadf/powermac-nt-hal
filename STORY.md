...
| 11 | ~~`SYSTEMPARTITION` synthesised by the HAL from the loader's ARC disk list (wall 35)~~ **retired (16 September)** | on real ARC hardware it is NVRAM, written by `ARCINST.EXE`; the HAL guessing it is policy in the wrong place | done: the veneer reads every ARC variable as a property of Open Firmware's `/options` node -- which *is* this machine's NVRAM -- and `mkbootfloppy.py`'s `SETUP.OF` stores the ten of them with `setenv`, once per machine (the boot-floppy note, §2.3). The HAL's synthesis stays as a fallback for a machine that has not run it |
| 12 | ~~The ARC environment does not survive a reboot~~ **retired (16 September)** | it was plain memory in the HAL | done: it is NVRAM now, see row 11 -- `setenv OSLOADER …` survives `reset-all` (probed) |
