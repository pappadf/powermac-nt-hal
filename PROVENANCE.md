...
| `tools/mkoem.py`, `mkarcdisk.py`, `mkbootscript.py`, `mkveneer.py`, `mkcoldboot.py`, `mkpatchediso.py`, `mkbootfloppy.py`, `coffsyms.py`, `coffdis.py`, `fatls.py`, `fatcat.py`, `fatput.py`, `savedisk.py`, `mkbootcd.py`, `gen-probe.py` | original tools. The formats they read and write are public: ISO 9660, MBR, FAT12/FAT16, PE/COFF and the NT registry hive. The addresses they patch were found by disassembling the user's own binaries with `coffdis.py` / `pe-dis.py`; no code or data from those binaries is stored here, and every file they place on an image is named on the command line by the user | own work | original | 2026-09-15 | pappadf using Claude |
| `tools/mkbootfloppy.py` (the `txtsetup.oem` template) | the section names (`Disks`, `Defaults`, `Computer`, `Keyboard`, `Display`, `Files.`, `Config.`, `Strings`) and key names (`hal`, `driver`, `port`, `dll`, `inf`) read out of the shipped `SETUPLDR`'s string table beside `D:\nt\private\ntos\boot\setup\oemdisk.c`; the shape of each entry read from the CD's own `TXTSETUP.SIF` | Microsoft binary and data file, read not copied; the names are an interface an OEM disk must use or Setup does not read it | **rewritten from reading**: no line of either file is reproduced; what is used is which words Setup looks for | 2026-09-15 | pappadf using Claude |
