...
| `mkbootfloppy.py` | builds the 1.44 MB FAT12 boot floppy of [`docs/2026-09-15-the-boot-floppy.md`](../docs/2026-09-15-the-boot-floppy.md) — the patched veneer, `SETUPLDR`, this project's HAL as `HALSHINR.DLL`, the ADB keyboard driver and a `txtsetup.oem`. The veneer is allocated **first**, so it starts at cluster 2 and is contiguous: Open Firmware reads it with `read-blocks` off the raw device before any filesystem is parsed, and the tool prints the block and length to pass to `mkcoldboot.py`. `--display-driver`/`--display-dll`/`--vga-aperture` add a `[Display]` class whose miniport claims the legacy VGA aperture at an address this board can actually translate — which is what clears wall 25 without a poke. Every file is named on the command line and comes from the user's own media |
